...
void Transform::easeTo(const CameraOptions& camera, const AnimationOptions& animation) {
    ...
    if (state.getLatLngBounds() == LatLngBounds::unbounded() && !isGestureInProgress() && duration != Duration::zero()) {
    ...
    const LatLng& latLng = state.getLatLngBounds() != LatLngBounds::unbounded() ? unwrappedLatLng : unwrappedLatLng.wrapped();
    ...
    if (state.getLatLngBounds() == LatLngBounds::unbounded()) {
